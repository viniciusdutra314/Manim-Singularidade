...
def number_to_bytes(number):
    str_formats={float64:'d',float:'d',float32:'f',float16:'e'}
...
def preaty_print_float(number,exact_representation):
    ...
    bits_info={float16:(16,5),float32:(32,8),float64:(64,11),float:(64,11)}
    ...
    print(f"[red]{sign_bit}[/red][green]{binary_exponent}[/green][bright_yellow]{binary_mantissa}[/bright_yellow]")
    print('')
    print(f"Sinal 1-bit: [red]{sign_bit}[/red] {'(positivo)' if sign_bit else 'negativo'}")
    print('')
    print(f"Expoente {bits_exp}-bits: [green]{binary_exponent}= {exponent+exp_bias}-{exp_bias}=[/green]{exponent}")
    print('')
    print(f"Mantissa {bits_mantissa}-bits [bright_yellow]{bytes_str[bits_exp+1:]}[/bright_yellow] = {mantissa:.{bits_mantissa}f}")
    print("----------------")
    print(f"{exact_representation}(Float-{bits_total})=[bright_yellow]{mantissa:.{bits_mantissa}f}[/bright_yellow]*(2^[green]{exponent}[/green])={mantissa*2**exponent:.{bits_total}f}")
x=float64(0.3)
preaty_print_float(x,'0.3')
